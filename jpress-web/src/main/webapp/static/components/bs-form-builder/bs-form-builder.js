...
(function ($) {
  ...
  var defaultOptions = {
    ...
    actionButtons: [
      {
        ...
        onclick: function (event, builder) {
          ...
          var html = builder._renderTemplate(
            modal,
            ["title", "content"],
            ["json内容", json]
          );
    ...
    actionButtonTemplate:
      '<button type="button" class="btn btn-sm {{mainClass}}" >' +
      '  <i class="{{icon}}"></i>{{text}}' +
      "</button>",
    templateLoadUrl: "",
    templateItemTemplate:
      '<div class="bs-template-item" id="{{id}}">' +
  ...
  var defaultComponents = [
    ...
    //等分栅格
    {
      ...
      onPropChange: function (bsFormBuilder, data, propName, value) {
        ...
        $row
          .children()
          .attr("class", "bsItemContainer col-" + 12 / Number.parseInt(value));
        return true;
      },
    },
  ...
  var BsFormBuilder = function (element, options) {
    ...
    for (let defaultProp of defaultProps) {
      if (
        this.defaultProps.map((item) => item.name).indexOf(defaultProp.name) ===
        -1
      ) {
  ...
  BsFormBuilder.prototype = {
    ...
    _initViewMode: function () {
      ...
      if (this.$container.length === 0) {
        throw new Error(
          "Can not file container by: " +
            this.options.bsFormContainerFilterSelector
        );
      }
    ...
    _initBuilderMode: function () {
      ...
      if (this.$container.length === 0) {
        throw new Error(
          "Can not file container by: " + this.options.bsFormContainerSelector
        );
      }

      this.$containerPlaceHolder = this.$rootEl.find(
        this.options.bsFormContainerPlaceHolderSelector
      );

      this.$propsPanelTitle = this.$rootEl.find(
        this.options.bsFormPropsTitleSelector
      );
      ...
      this._initComponents(function () {
        ...
        // url 请求
        if (
          typeof optionDatas === "string" &&
          bsFormBuilder._isUrl(optionDatas)
        ) {
        ...
        //直接配置数组
          ...
          typeof optionDatas === "object" &&
          Array.isArray(optionDatas)
        ) {
          invoker(optionDatas);
        }
    ...
    _initViewStructure: function () {
      this.$rootEl.append(
        '<div class="row bsFormViewRoot"><div class="col-12 bsFormContainer"></div></div>'
      );
    },
    ...
    _initBuilderStructure: function () {
      this.$rootEl.append(
        '<div class="row bsFormBuilderRoot">' +
          "  <!--左侧拖拽区域-->" +
    ...
    _initComponents: function (initAfterFunc) {
      ...
      for (let component of defaultComponents) {
        if (
          ...
          (useComponents.length === 0 ||
            useComponents.indexOf(component.tag) > -1) &&
          unUseComponents.indexOf(component.tag) === -1
        ) {
      ...
      var initComponents = function (customComponents) {
        ...
        for (let component of customComponents) {
          if (component && component.tag) {
            component = $.extend(
              bsFromBuilder.components[component.tag],
              component
            );
      ...
      // url 请求
      if (
        typeof customComponents === "string" &&
        this._isUrl(customComponents)
      ) {
      ...
      //直接配置数组
        ...
        typeof customComponents === "object" &&
    ...
    _initDragComponents: function () {
      ...
      for (let component of Object.values(this.components)) {
        ...
        if (component.drag.type) {
          ...
          console.error(
            "Component define error! it must defined drag.type in component. ",
            component
          );
        }
      }
      ...
      $(".bsFormDrags").each(function (index, element) {
        ...
        //没有找到该类型的任何组件
          ...
          if (
            bsFormBuilder.options.renderEmptyDrags &&
            typeof bsFormBuilder.options.renderEmptyDrags === "function"
          ) {
    ...
    _initEvents: function () {
      ...
      this.$propsPanel.on("click", ".option-add", function (event) {
        var options =
          bsFormBuilder.deepCopy(bsFormBuilder.currentData.options, false) ||
          [];
        ...
        if (bsFormBuilder.currentData.component.opiontsAdd) {
          if (
            typeof bsFormBuilder.currentData.component.opiontsAdd === "function"
          ) {
            newOption =
              bsFormBuilder.currentData.component.opiontsAdd(bsFormBuilder);
          } else {
        ...
        bsFormBuilder.updateDataAttr(
          bsFormBuilder.currentData,
          "options",
          options
        );
    ...
    _initDataOptionsIfNecessary: function (data) {
      if (data.component.withOptions && !data.options) {
        ...
        if (typeof defaultOptions === "function") {
          defaultOptions = data.component.defaultOptions(
            this,
            this.currentData
          );
        }
    ...
    _syncCurrentDataOptionsFromPropSetting: function () {
      var options = [];
      var optionItems = this.$propsPanel
        .children(".options")
    ...
    createComponentData: function (component) {
      var data =
        component.onDataCreate && typeof component.onDataCreate === "function"
    ...
    renderDefault: function (data) {
      ...
      var allPropNames = this.defaultProps
        .map((prop) => prop.name)
        .concat(["value", "placeholder", "options"]);
      if (data.component.props) {
        allPropNames = allPropNames.concat(
          data.component.props.map((prop) => prop.name)
        );
      }

      var paras = ["$builder", "$component", "$data", "$children"].concat(
        allPropNames
      );
        ...
        for (let key in target) {
          if (target.hasOwnProperty(key)) {
            if (target[key] && typeof target[key] === "object") {
              newObject[key] = this.deepCopy(
                target[key],
                withNewElementIdAndId
              );
            } else {
              var value = target[key];
              if (
                withNewElementIdAndId &&
                (key === "elementId" || key === "id")
              ) {
    ...
    render: function (data, withActive) {
      ...
      if (typeof component.template === "function") {
        ...
        if (typeof this.options.customRender === "function") {
          ...
          typeof this.options.customRender === "string" &&
      ...
      if (withActive) {
        this._makeElementActive(
          $template,
          this.currentData.component.disableTools
        );
      }
    ...
    getParentArrayByElementIdInArray: function (array, elementId) {
      ...
      for (let i = 0; i < array.length; i++) {
        ...
        if (item && item.elementId === elementId) {
          ...
          for (let childArray of Object.values(item.children)) {
            let ret = this.getParentArrayByElementIdInArray(
              childArray,
              elementId
            );
            if (ret) return ret;
    ...
    refreshPropsPanel: function () {
      ...
      if (this.$propsPanelTitle.length > 0) {
        ...
        if (component.drag.icon) {
          ...
          if (
            component.drag.icon &&
            component.drag.icon.trim().charAt(0) === "<"
          ) {
      ...
      if (this.currentData.component.withOptions || this.currentData.options) {
        ...
        if (typeof componentSupportTypes === "string") {
          ...
          componentSupportTypes = this.currentData.component.optionsTypes(
            this,
            this.currentData
          );
        }
        ...
        if (
          ...
          componentSupportTypes.length > 1
        ) {
          let prop = {
            id: this.genRandomId(),
            options: [
    ...
    _appendPropsPanel: function (prop, data, template) {
      ...
      if (
        this.options.bsFormPropsItemAppendBefore &&
        typeof this.options.bsFormPropsItemAppendBefore === "function"
      ) {
        htmlString = this.options.bsFormPropsItemAppendBefore(
          this,
          prop,
          data,
          htmlString
      ...
      if (
        this.options.bsFormPropsItemAppended &&
        typeof this.options.bsFormPropsItemAppended === "function"
      ) {
    ...
    _getComponentAllProps: function (component, data) {
      ...
      //组件定义使用的属性
      let useProps =
        typeof component.useProps === "function"
          ? component.useProps(this, data)
          : typeof component.useProps === "object"
          ? component.useProps
          : [];
      ...
      //组件定义的 "私有" 属性
      var componentProps =
        typeof component.props === "object" ? component.props : [];
      ...
      //过滤属性
      if (
        this.options.bsFormPropsFilter &&
        typeof this.options.bsFormPropsFilter === "function"
      ) {
    ...
    updateDataAttr: function (data, attr, value) {
      ...
      if (
        data.component &&
        typeof data.component.onPropChange === "function" &&
      ...
      //配置 onDataChange 方法
        ...
        typeof this.options.onDataChange === "function" &&
      ...
      if ("optionsType" === attr && data === this.currentData) {
        ...
        if (value === "datasource") {
          ...
          var optionsDatasource =
            data.optionsDatasource || this._getOptionDatasources()[0].value;
  ...
  $.fn.bsFormBuilder = function (option) {
    ...
    return this.each(function () {
      ...
      if (!bsFormBuilder) {
        $this.data(
          "bsFormBuilder",
          (bsFormBuilder = new BsFormBuilder(this, options))
        );
      }
